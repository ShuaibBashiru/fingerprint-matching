...
def list_record(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.date_in FROM user_record t1 INNER JOIN attendance t2 ON "
                                     "t1.id=t2.user_id WHERE t1.record_status=%s LIMIT %s OFFSET %s", [1, limitTo,
                                                                                                       offset])
...
def list_filter(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.date_in FROM user_record t1 INNER JOIN attendance t2 ON "
                                     "t1.id=t2.user_id WHERE "
                                     "t1.record_status=%s AND t1.status_id=%s ", [1, statusid])
            row = dictfetchall(cursor)
            if counter > 0:
                feedback = {
                    'status': 'success',
...
def list_search(request):
    try:
        ...
        with connection.cursor() as cursor:
            counter = cursor.execute(" t1.*, t2.date_ine FROM user_record t1 INNER JOIN attendance t2 ON "
                                     "t1.id=t2.user_id WHERE t1.record_status=%s AND t1.personal_id like %s OR "
                                     "t1.email_one like %s OR t1.surname like %s ",
...
def download(request):
    try:
        with connection.cursor() as cursor:
            counter = cursor.execute("SELECT t1.*, t2.date_in FROM user_record as t1 INNER JOIN attendance t2 "
                                     "ON t1.id=t2.user_id WHERE t1.status_id=%s", [1, ])
